 ...
 *
 * **Important:** The plugin must be constructed *inside* the `Javalin.create()` config lambda
 * so it can capture the config consumer via StackWalker. If constructed outside (e.g., as a field),
 * set [Config.onReload] explicitly.
 *
 * Usage:
 * ```java
...
class DevReloadPlugin(userConfig: Consumer<Config>? = null) : Plugin<DevReloadPlugin.Config>(userConfig, Config()) {
    ...
    // The name and declaring class of the lambda method that backs the original Javalin.create() consumer.
    // Captured at construction time while the user's config lambda is still on the stack.
    private val originalLambdaInfo: Pair<String, String>? = StackWalker.getInstance().walk { frames ->
        frames.filter { frame -> frame.methodName.contains("lambda") }
            .map { frame -> frame.methodName to frame.className }
            .findFirst()
            .orElse(null)
    }
    ...
    class Config {
        ...
        /**
         * Minimum time in milliseconds between filesystem walks.
         * The watcher skips the directory walk if called again within this cooldown period.
         * Default is 500ms, which avoids redundant walks on burst HTTP requests.
         */
        @JvmField
        var watchCooldownMs: Long = 500
    }
    ...
    override fun onStart(state: JavalinState) {
        val reloadConsumer = pluginConfig.onReload ?: captureUserConfig(state)

        if (reloadConsumer == null) {
            JavalinLogger.warn(
                "DevReloadPlugin: Could not auto-detect the Javalin.create() config consumer. " +
                "This usually means DevReloadPlugin was constructed outside the Javalin.create() lambda. " +
                "Either construct it inline: config.registerPlugin(new DevReloadPlugin()), " +
                "or set onReload explicitly: new DevReloadPlugin(c -> c.onReload = config -> { ... })"
            )
            return
        }
     ...
     * The watcher rate-limits filesystem walks via cooldown, so this is cheap when called frequently.
    ...
    private fun checkAndReload(
        ...
        // Single atomic check — walks filesystem at most once, updates baseline immediately
    ...
    internal fun reload(
        ...
        try {
            ...
            DevReloadReflection.withPluginReloadingEnabled(state) {
                ...
                val publicConfig = createJavalinConfig(state)
    ...
    private fun reloadConsumerFromDisk(originalConsumer: Consumer<JavalinConfig>): Consumer<JavalinConfig> {
        try {
            ...
            // Use the user's class (captured at construction time) as the enclosing class to reload.
            // originalLambdaInfo.second is the class where the Javalin.create() config lambda lives.
            // Falling back to deriving it from the consumer wrapper class name.
            val enclosingClassName = originalLambdaInfo?.second ?: run {
                val lambdaClassName = originalConsumer.javaClass.name
                if ("$$" in lambdaClassName) lambdaClassName.substringBefore("$$") else lambdaClassName
            }
            ...
            var targetMethodName = originalLambdaInfo?.first
     ...
     * names captured at construction time. This avoids requiring a `userConfig` field on
     * the core JavalinState class.
     */
    private fun captureUserConfig(state: JavalinState): Consumer<JavalinConfig>? {
        val (methodName, className) = originalLambdaInfo ?: return null
        try {
            val clazz = Class.forName(className)
            val method = clazz.declaredMethods.find { it.name == methodName }
            if (method != null) {
                method.isAccessible = true
                logExtensive("DevReloadPlugin: Captured config lambda '$methodName' from $className")
                return Consumer { config -> method.invoke(null, config) }
            }
        } catch (e: Exception) {
            logExtensive("DevReloadPlugin: Could not capture config lambda: ${e.message}")
        }
        return null
    }

    /** Creates a JavalinConfig via reflection (its constructor is internal to the javalin module). */
    private fun createJavalinConfig(state: JavalinState): JavalinConfig {
