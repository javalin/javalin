...
internal object DevReloadReflection {
     ...
     * Swaps the PluginManager's internal `plugins`, `initializedPlugins`, and `enabledPlugins`
     * lists with empty guarded wrappers that silently ignore add() for already-registered plugin
     * types. This bypasses the duplicate check in PluginManager.register() and prevents
     * re-initialization/re-start of existing plugins.
     */
    fun <T> withPluginReloadingEnabled(state: JavalinState, block: () -> T): T {
        val pluginManager = state.pluginManager
        val clazz = pluginManager.javaClass
        val pluginsField = clazz.getDeclaredField("plugins").apply { isAccessible = true }
        val initializedField = clazz.getDeclaredField("initializedPlugins").apply { isAccessible = true }
        val enabledField = clazz.getDeclaredField("enabledPlugins").apply { isAccessible = true }

        val originalPlugins = pluginsField.get(pluginManager) as MutableList<Any>
        val originalInitialized = initializedField.get(pluginManager) as MutableList<Any>
        val originalEnabled = enabledField.get(pluginManager) as MutableList<Any>
        val existingClasses = originalPlugins.map { it.javaClass }.toSet()

        // Empty lists that silently skip add() for already-registered plugin types
        fun guardedList() = object : ArrayList<Any>() {
            override fun add(element: Any): Boolean {
                if (element.javaClass in existingClasses) return true // silently skip duplicate
                return super.add(element)
            }
        }
        pluginsField.set(pluginManager, guardedList())
        initializedField.set(pluginManager, guardedList())
        enabledField.set(pluginManager, guardedList())

        try {
            return block()
        } finally {
            pluginsField.set(pluginManager, originalPlugins)
            initializedField.set(pluginManager, originalInitialized)
            enabledField.set(pluginManager, originalEnabled)
        }
    }
