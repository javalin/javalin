...
data class WsEntry(val path: String, val handler: WsHandler, private val lowerCasePaths:Boolean ) {
 ...
 */
@WebSocket
class WsPathMatcher( private val lowerCasePaths:Boolean ) {
    ...
    private fun wrap(session: Session, wsEntry: WsEntry): WsSession {
        ...
        return WsSession(sessionIds[session]!!, session, sessionPathParams[session]!!, wsEntry.path, lowerCasePaths)
    }
