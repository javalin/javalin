...
object SeekableWriter {
    ...
    fun write(ctx: Context, inputStream: InputStream, contentType: String) {
        ...
        val to = when {
            from + chunkSize > totalBytes -> totalBytes // chunk bigger than file, write all
            requestedRange.size == 2 -> min(requestedRange[1].toInt(), totalBytes) // chunk smaller than file, to/from specified
        ...
        ctx.header(Header.CONTENT_LENGTH, "${to - from}")
    ...
    private fun OutputStream.write(inputStream: InputStream, from: Int, to: Int, buffer: ByteArray = ByteArray(1024)) = inputStream.use {
        it.skip(from.toLong())
        var bytesLeft = to - from
        while (bytesLeft != 0) {
            val read = it.read(buffer, 0, min(buffer.size, bytesLeft))
