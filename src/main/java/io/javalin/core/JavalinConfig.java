...
public class JavalinConfig {
  ...
  public JavalinConfig addSinglePageRoot(
      @NotNull String path, @NotNull String filePath, @NotNull Location location) {
  ...
  public static void applyUserConfig(
      Javalin app, JavalinConfig config, Consumer<JavalinConfig> userConfig) {
    ...
    config
        .getPluginsExtending(PluginLifecycleInit.class)
        .forEach(
            plugin -> {
              plugin.init(app);
  ...
  private <T> Stream<? extends T> getPluginsExtending(Class<T> clazz) {
    return inner.plugins.values().stream().filter(clazz::isInstance).map(plugin -> (T) plugin);
  }
}
