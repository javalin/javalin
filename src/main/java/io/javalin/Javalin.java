...
public class Javalin {
  ...
  /**
   * Do not use this field unless you know what you're doing. Application config should be declared
   * in {@link Javalin#create(Consumer)}
   */
  ...
  /**
   * Creates a new instance with the user provided configuration. The server does not run until
   * {@link Javalin#start()} is called.
   *
  ...
  public static Javalin create(Consumer<JavalinConfig> config) {
    ...
    JavalinConfig.applyUserConfig(
        app, app.config, config); // mutates app.config and app (adds http-handlers)
  ...
  public Javalin start() {
    ...
    try {
      ...
      if (e.getMessage() != null && e.getMessage().contains("Failed to bind to")) {
        ...
        throw new RuntimeException(
            "Port 1-1023 require elevated privileges (process must be started by admin).", e);
      }
  ...
  /**
   * Get which port instance is running on Mostly useful if you start the instance with port(0)
   * (random port)
   */
  public int port() {
  ...
  /**
   * Registers an attribute on the instance. Instance is available on the {@link Context} through
   * {@link Context#appAttribute}. Ex: app.attribute(MyExt.class, myExtInstance()) The method must
   * be called before {@link Javalin#start()}.
   */
  ...
  /**
   * Retrieve an attribute stored on the instance. Available on the {@link Context} through {@link
   * Context#appAttribute}. Ex: app.attribute(MyExt.class).myMethod() Ex:
   * ctx.appAttribute(MyExt.class).myMethod()
   */
  ...
  /**
   * Creates a temporary static instance in the scope of the endpointGroup. Allows you to call
   * get(handler), post(handler), etc. without without using the instance prefix.
   *
   * @see <a href="https://javalin.io/documentation#handler-groups">Handler groups in
   *     documentation</a>
   * @see ApiBuilder
   */
   ...
   */
  public <T extends Exception> Javalin exception(
      @NotNull Class<T> exceptionClass, @NotNull ExceptionHandler<? super T> exceptionHandler) {
    servlet
        .getExceptionMapper()
        .getHandlers()
        .put(exceptionClass, (ExceptionHandler<Exception>) exceptionHandler);
    return this;
  }

  /**
   * Adds an error mapper to the instance. Useful for turning error-codes (404, 500) into
   * standardized messages/pages
   *
  ...
  /**
   * Adds an error mapper for the specified content-type to the instance. Useful for turning
   * error-codes (404, 500) into standardized messages/pages
   *
   * @see <a href="https://javalin.io/documentation#error-mapping">Error mapping in docs</a>
   */
  ...
  /**
   * Adds a request handler for the specified handlerType and path to the instance. Requires an
   * access manager to be set on the instance. This is the method that all the verb-methods
   * (get/post/put/etc) call.
   *
   * @see AccessManager
   * @see <a href="https://javalin.io/documentation#handlers">Handlers in docs</a>
   */
  public Javalin addHandler(
      @NotNull HandlerType handlerType,
      @NotNull String path,
  ...
  /**
   * Adds a request handler for the specified handlerType and path to the instance. This is the
   * method that all the verb-methods (get/post/put/etc) call.
   *
   * @see <a href="https://javalin.io/documentation#handlers">Handlers in docs</a>
   */
  public Javalin addHandler(
      @NotNull HandlerType httpMethod, @NotNull String path, @NotNull Handler handler) {
    return addHandler(
        httpMethod,
        path,
        handler,
        new HashSet<>()); // no roles set for this route (open to everyone with default access
                          // manager)
  }
  ...
  /**
   * Adds a lambda handler for a Server Sent Event connection on the specified path. Requires an
   * access manager to be set on the instance.
   */
  public Javalin sse(
      @NotNull String path,
   ...
   * @see <a href="https://javalin.io/documentation#exception-mapping">Exception mapping in docs</a>
   */
  public <T extends Exception> Javalin wsException(
      @NotNull Class<T> exceptionClass, @NotNull WsExceptionHandler<? super T> exceptionHandler) {
    wsServlet
        .getWsExceptionMapper()
        .getHandlers()
        .put(exceptionClass, (WsExceptionHandler<Exception>) exceptionHandler);
    return this;
  }

  /**
   * Adds a specific WebSocket handler for the given path to the instance. Requires an access
   * manager to be set on the instance.
   */
  private Javalin addWsHandler(
      @NotNull WsHandlerType handlerType,
      @NotNull String path,
      @NotNull Consumer<WsHandler> wsHandler,
      @NotNull Set<Role> roles) {
    wsServlet.addHandler(handlerType, path, wsHandler, roles);
    eventManager.fireWsHandlerAddedEvent(
        new WsHandlerMetaInfo(
            handlerType,
            Util.prefixContextPath(servlet.getConfig().contextPath, path),
            wsHandler,
            roles));
  ...
  /**
   * Adds a WebSocket handler on the specified path with the specified roles. Requires an access
   * manager to be set on the instance.
   *
   * @see AccessManager
   * @see <a href="https://javalin.io/documentation#websockets">WebSockets in docs</a>
