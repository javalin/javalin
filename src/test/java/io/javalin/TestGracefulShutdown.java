...
public class TestGracefulShutdown {
    ...
    public void t1_shutdown_is_graceful_with_default_config() throws Exception {
        Javalin app = sharedJavalinConfiguration().start(0);
    ...
    public void t2_shutdown_is_graceful_when_custom_server_has_statisticshandler() throws Exception {
        Javalin app = sharedJavalinConfiguration().configure(c -> c.server(() -> {
    ...
    public void t3_shutdown_is_not_graceful_when_custom_server_has_no_statisticshandler() throws Exception {
        Javalin app = sharedJavalinConfiguration().configure(c -> c.server(Server::new)).start(0);
    ...
    private Javalin sharedJavalinConfiguration() {
        return Javalin.create().disableStartupBanner()
