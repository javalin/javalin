...
class TestStaticFiles {
    ...
    @Test
    fun test_Html() = TestUtil.test(defaultStaticResourceApp) { app, http ->
    ...
    @Test
    fun test_getJs() = TestUtil.test(defaultStaticResourceApp) { app, http ->
    ...
    @Test
    fun test_getCss() = TestUtil.test(defaultStaticResourceApp) { app, http ->
    ...
    @Test
    fun test_beforeFilter() = TestUtil.test(defaultStaticResourceApp) { app, http ->
    ...
    @Test
    fun test_rootReturns404_ifNoWelcomeFile() = TestUtil.test(defaultStaticResourceApp) { app, http ->
        assertThat(http.get("/").code(), `is`(404))
        assertThat(http.getBody("/"), `is`("Not found"))
    }

    @Test
    fun test_rootReturnsWelcomeFile_ifWelcomeFileExists() = TestUtil.test(defaultStaticResourceApp) { app, http ->
        assertThat(http.get("/subdir/").code(), `is`(200))
        assertThat(http.getBody("/subdir/"), `is`("<h1>Welcome file</h1>"))
    }

    @Test
    fun test_expiresWorksAsExpected() = TestUtil.test(defaultStaticResourceApp) { app, http ->
    ...
    @Test
    fun test_externalFolder() = TestUtil.test(externalStaticResourceApp) { app, http ->
        assertThat(http.get("/html.html").code(), `is`(200))
        assertThat(http.getBody("/html.html"), containsString("HTML works"))
    }

    @Test
    fun test_multipleCallsToEnableStaticFiles() = TestUtil.test(multiLocationStaticResourceApp) { app, http ->
