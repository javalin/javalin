        ...
        ((server, httpUtil) -> {
          ...
          Headers headers =
              httpUtil
                  .sse(ssePath)
                  .get()
                  .getHeaders(); // Headers extends HashMap<String, List<String>>
          assertTrue(headers.containsKey("Connection"));
          assertTrue(headers.containsKey("Cache-Control"));
          assertTrue(headers.containsKey("Content-Type"));
          String connection =
              headers.get("Connection").get(headers.get("Connection").size() - 1).toLowerCase();
          String contentType =
              headers.get("Content-Type").get(headers.get("Content-Type").size() - 1).toLowerCase();
          String cacheControl =
              headers
                  .get("Cache-Control")
                  .get(headers.get("Cache-Control").size() - 1)
                  .toLowerCase();
