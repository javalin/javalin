...
class JavaCrudHandler implements CrudHandler {
  @OpenApi(
      responses = {
        @OpenApiResponse(
            status = "200",
            content = @OpenApiContent(from = User.class, isArray = true))
      })
  @Override
...
class ClassHandlerWithInvalidPath {
  ...
  @OpenApi(method = HttpMethod.GET, path = "/account")
...
public class TestOpenApiAnnotations_Java {
  ...
  public void testWithSimpleExample() {
    OpenApiOptions openApiOptions =
        new OpenApiOptions(new Info().title("Example").version("1.0.0"));
    Javalin app =
  ...
  public void testWithJavaMethodReference() {
    ...
    OpenAPI schema =
        OpenApiTestUtils.extractSchemaForTest(
            options,
            app -> {
  ...
  public void testWithJavaStaticMethodReference() {
    ...
    OpenApiOptions options =
        new OpenApiOptions(info).activateAnnotationScanningFor("io.javalin.openapi");
  ...
  public void testIfUserIsWarnedOnInvalidPath() {
    String log =
        TestLoggingUtilKt.captureStdOut(
            () -> {
              OpenApiTestUtils.extractSchemaForTest(
                  app -> {
