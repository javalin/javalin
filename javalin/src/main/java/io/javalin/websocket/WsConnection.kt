...
class WsConnection(val matcher: WsPathMatcher, val exceptionMapper: WsExceptionMapper, val wsLogger: WsConfig?) {
    ...
    fun onConnect(session: Session) {
        ...
        tryBeforeAndEndpointHandlers(ctx) { it.config.wsConnectHandler?.handleConnect(ctx) }
        tryAfterHandlers(ctx) { it.config.wsConnectHandler?.handleConnect(ctx) }
    ...
    fun onMessage(session: Session, message: String) {
        ...
        tryBeforeAndEndpointHandlers(ctx) { it.config.wsMessageHandler?.handleMessage(ctx) }
        tryAfterHandlers(ctx) { it.config.wsMessageHandler?.handleMessage(ctx) }
    ...
    fun onMessage(session: Session, buffer: ByteArray, offset: Int, length: Int) {
        ...
        tryBeforeAndEndpointHandlers(ctx) { it.config.wsBinaryMessageHandler?.handleBinaryMessage(ctx) }
        tryAfterHandlers(ctx) { it.config.wsBinaryMessageHandler?.handleBinaryMessage(ctx) }
    ...
    fun onClose(session: Session, statusCode: Int, reason: String?) {
        ...
        tryBeforeAndEndpointHandlers(ctx) { it.config.wsCloseHandler?.handleClose(ctx) }
        tryAfterHandlers(ctx) { it.config.wsCloseHandler?.handleClose(ctx) }
    ...
    fun onError(session: Session, throwable: Throwable?) {
        ...
        tryBeforeAndEndpointHandlers(ctx) { it.config.wsErrorHandler?.handleError(ctx) }
        tryAfterHandlers(ctx) { it.config.wsErrorHandler?.handleError(ctx) }
