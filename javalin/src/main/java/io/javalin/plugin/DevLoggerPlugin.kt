...
private fun resBody(ctx: Context): String {
    val staticFile = ctx.request().getAttribute("handled-as-static-file") == true
    if (staticFile) {
    ...
    val gzipped = ctx.response().getHeader(Header.CONTENT_ENCODING) == "gzip"
    val brotlied = ctx.response().getHeader(Header.CONTENT_ENCODING) == "br"
