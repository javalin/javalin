...
class TestFilters {
    ...
    fun `filters run before root handler`() = TestUtil.test { app, http ->
        app.unsafe.routes.before { it.header("X-BEFOREFILTER", "Before-filter ran") }
        app.unsafe.routes.after { it.header("X-AFTERFILTER", "After-filter ran") }
        app.unsafe.routes.get("/", TestUtil.okHandler)
    ...
    fun `app returns 404 for GET if only before-handler present`() = TestUtil.test { app, http ->
        app.unsafe.routes.before(TestUtil.okHandler)
    ...
    fun `before-handler can set header`() = TestUtil.test { app, http ->
        app.unsafe.routes.before { it.header("X-FILTER", "Before-filter ran") }
        app.unsafe.routes.get("/mapped", TestUtil.okHandler)
    ...
    fun `before- and after-handlers can set a bunch of headers`() = TestUtil.test { app, http ->
        app.unsafe.routes.before { it.header("X-BEFORE-1", "Before-filter 1 ran") }
        app.unsafe.routes.before { it.header("X-BEFORE-2", "Before-filter 2 ran") }
        app.unsafe.routes.after { it.header("X-AFTER-1", "After-filter 1 ran") }
        app.unsafe.routes.after { it.header("X-AFTER-2", "After-filter 2 ran") }
        app.unsafe.routes.get("/mapped", TestUtil.okHandler)
    ...
    fun `after-handler sets header after endpoint-handler`() = TestUtil.test { app, http ->
        app.unsafe.routes.get("/mapped", TestUtil.okHandler)
        app.unsafe.routes.after { it.header("X-AFTER", "After-filter ran") }
    ...
    fun `after is after before`() = TestUtil.test { app, http ->
        app.unsafe.routes.before { it.header("X-FILTER", "This header is mine!") }
        app.unsafe.routes.after { it.header("X-FILTER", "After-filter beats before-filter") }
        app.unsafe.routes.get("/mapped", TestUtil.okHandler)
