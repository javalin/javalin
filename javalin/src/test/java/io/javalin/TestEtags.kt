...
class TestEtags {
    ...
    fun `default app does not set etags for GET`() = TestUtil.test { app, http ->
        ...
        assertThat(Unirest.get(http.origin + "/").asString().headers.getFirst(Header.ETAG.name)).isNullOrEmpty()
    }
    ...
    fun `autogenerated etags are added to GET request`() = TestUtil.test(etagsEnabledApp) { app, http ->
        ...
        val etag = response.headers.getFirst(Header.ETAG.name)
        val response2 = Unirest.get(http.origin + "/automatic").header(Header.IF_NONE_MATCH.name, etag).asString()
        val etag2 = response2.headers.getFirst(Header.ETAG.name)
    ...
    fun `autogenerated etags are not added to PUT request`() = TestUtil.test(etagsEnabledApp) { app, http ->
        ...
        assertThat(response.headers.getFirst(Header.ETAG.name)).isNullOrEmpty()
    }
    ...
    fun `manual etags overwrite autogenerated etags`() = TestUtil.test(etagsEnabledApp) { app, http ->
        ...
        assertThat(response.body).isEqualTo("Hello!")
        val etag = response.headers.getFirst(Header.ETAG.name)
        val response2 = Unirest.get(http.origin + "/manual").header(Header.IF_NONE_MATCH.name, etag).asString()
        assertThat(response2.status).isEqualTo(304)
        assertThat(response2.body).isNullOrEmpty()
    ...
    fun `manual etags work for PUT request`() = TestUtil.test { app, http ->
        ...
        assertThat(response.headers.getFirst(Header.ETAG.name)).isEqualTo("1234")
    }
