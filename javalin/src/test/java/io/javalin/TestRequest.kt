...
class TestRequest {
    ...
    fun `session-attributes work`() = TestUtil.test { app, http ->
        app.get("/store-session") { it.request().session.setAttribute("test", "tast") }
        app.get("/read-session") { it.result(it.request().session.getAttribute("test") as String) }
    ...
    fun `session-attribute retrieval does not create session`() = TestUtil.test { app, http ->
        ...
        app.get("/has-session") { it.result("" + (it.request().getSession(false) != null)) }
    ...
    fun `servlet-context is not null`() = TestUtil.test { app, http ->
        app.get("/") { it.result(if (it.request().servletContext != null) "not-null" else "null") }
