...
class TestExceptionMapper {
    ...
    fun `unmapped exceptions are caught by default handler`() = TestUtil.test { app, http ->
        app.unsafe.routes.get("/unmapped-exception") { throw Exception() }
    ...
    fun `exceptions in exception handlers use default handler`() = TestUtil.test { app, http ->
        app.unsafe.routes.exception(Exception::class.java) { e, ctx -> throw IllegalArgumentException("Exception in exception handler") }
        app.unsafe.routes.get("/") { throw NumberFormatException() }
    ...
    fun `mapped exceptions are handled`() = TestUtil.test { app, http ->
        app.unsafe.routes.get("/mapped-exception") { throw Exception() }
    ...
    fun `HttpResponseException subclass handler is used`() = TestUtil.test { app, http ->
        app.unsafe.routes.get("/mapped-http-response-exception") { throw NotFoundResponse() }
    ...
    fun `type information of exception is not lost`() = TestUtil.test { app, http ->
        app.unsafe.routes.get("/typed-exception") { throw TypedException() }
    ...
    fun `most specific exception handler handles exception`() = TestUtil.test { app, http ->
        app.unsafe.routes.get("/exception-priority") { throw TypedException() }
    ...
    fun `catch-all Exception mapper doesn't override 404`() = TestUtil.test { app, http ->
        app.unsafe.routes.exception(Exception::class.java) { _, ctx -> ctx.status(INTERNAL_SERVER_ERROR) }
    ...
    fun `exceptions can have a lot of superclasses`() = TestUtil.test { app, http ->
        app.unsafe.routes.exception(Exception::class.java) { e, ctx -> ctx.result(e.javaClass.kotlin.allSuperclasses.size.toString()) }
        app.unsafe.routes.get("/") { throw NumberFormatException() }
    ...
    fun `exception title can contain quotes`() = TestUtil.test { app, http ->
        app.unsafe.routes.get("/") { throw BadRequestResponse("""MY MESSAGE WITH "QUOTES"""") }
    ...
    fun `exception title can contain newlines`() = TestUtil.test { app, http ->
        app.unsafe.routes.get("/") { throw BadRequestResponse("MY MESSAGE WITH \nNEWLINES\n") }
    ...
    fun `jetty eof exceptions are caught and handled as errors`() = TestUtil.test(Javalin.create {
        ...
        app.unsafe.routes.get("/") { throw EofException() }
    ...
    fun `jetty timeout exceptions are caught and handled as errors`() = TestUtil.test(Javalin.create {
        ...
        app.unsafe.routes.get("/") { throw IOException("timeout", TimeoutException()) }
    ...
    fun `errors are handled by javaLangErrorHandler`() = TestUtil.test { app, http ->
        app.unsafe.router.javaLangErrorHandler { res, throwable ->
            res.status = 200
            res.writer.write("Error: ${throwable.message}")
        }
        app.unsafe.routes.get("/") { throw OutOfMemoryError("sample") }
