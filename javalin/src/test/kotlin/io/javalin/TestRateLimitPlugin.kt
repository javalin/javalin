...
class TestRateLimitPlugin {
    ...
    @Test
    fun `rate limiting is scoped to path and method`() = TestUtil.test(
        Javalin.create { config ->
            config.registerPlugin(RateLimitPlugin { })
    ...
    @Test
    fun `rate limiter is cleared by scheduled executor`() = TestUtil.test(
        Javalin.create { config ->
            config.registerPlugin(RateLimitPlugin { })
            config.routes.get("/") { ctx ->
    ...
    @Test
    fun `custom key function can be configured`() = TestUtil.test(
        Javalin.create { config ->
    ...
    @Test
    fun `getCurrentCount returns correct count`() = TestUtil.test(
        Javalin.create { config ->
            config.registerPlugin(RateLimitPlugin { })
