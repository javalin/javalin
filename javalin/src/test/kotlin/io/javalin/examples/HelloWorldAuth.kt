...
fun main() {
    ...
    app.routes {
        ...
        path("/api") {
            ...
            get("/tast", { it.status(HttpCodes.OK).result("Hello world 3") }, ROLE_THREE)
            get("/hest", { it.status(HttpCodes.OK).result("Hello World 4") }, ROLE_ONE, ROLE_TWO)
            get("/hast", { it.status(HttpCodes.OK).result("Hello World 5").header("test", "tast") }, ROLE_ONE, ROLE_THREE)
        }
    }
...
private fun accessManager(handler: Handler, ctx: Context, routeRoles: Set<io.javalin.security.RouteRole>) {
    ...
    if (userRole != null && routeRoles.contains(MyRoles.valueOf(userRole))) {
        ...
        ctx.status(HttpCodes.UNAUTHORIZED).result("Unauthorized")
    }
}
