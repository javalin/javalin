...
class TestHttpAllowedMethodsPlugin {
    ...
    fun `enableHttpOptionsForRoutes allows possible methods on routes`() {
        ...
        JavalinTest.test(javalin) { app, http ->
            val response = http.options("/", mapOf(
                ACCESS_CONTROL_REQUEST_HEADERS to "123",
                ACCESS_CONTROL_REQUEST_METHOD to "TEST"
            ))

            assertThat(response.headers().get(ACCESS_CONTROL_ALLOW_METHODS)?.first())
                .isEqualTo("GET,DELETE,OPTIONS")

            val usersResponse = http.options("/users")

            assertThat(usersResponse.headers().get(ACCESS_CONTROL_ALLOW_METHODS)?.first())
