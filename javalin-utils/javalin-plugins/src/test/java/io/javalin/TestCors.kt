...
class TestCors {
    ...
    inner class HappyPath {
        @Test
        fun `can enable cors for specific origins`() = JavalinTest.test(Javalin.create {
        ...
        @Test
        fun `can enable cors for star origins`() = JavalinTest.test(Javalin.create {
            it.registerPlugin(CorsPlugin { cors ->
        ...
        @Test
        fun `has allowsCredentials false by default`() = JavalinTest.test(Javalin.create {
            it.registerPlugin(CorsPlugin { cors ->
        ...
        @Test
        fun `can have allowsCredentials set true`() = JavalinTest.test(Javalin.create { cfg ->
        ...
        fun `works for 404s`() =
            JavalinTest.test(Javalin.create {
                ...
                val optionsResponse = http.options("/not-found", mapOf(ORIGIN to "https://some-origin"))
                assertThat(optionsResponse.code())
                    .isEqualTo(200)
        ...
        @Test
        fun `works with options endpoint mapping`() = JavalinTest.test(Javalin.create {
            it.registerPlugin(CorsPlugin { cors ->
                cors.addRule { it.reflectClientOrigin = true }
            })
            it.routes.options("/") { it.result("Hello") }
        }) { app, http ->
            val response = http.options("/", mapOf(
    ...
    inner class NegativeTests {
        @Test
        fun `headers are not set when origin doesn't match`() = JavalinTest.test(Javalin.create { cfg ->
            cfg.registerPlugin(CorsPlugin { cors ->
        ...
        fun `same hostname with different ports is detected as different origins`() =
            JavalinTest.test(Javalin.create { cfg ->
                cfg.registerPlugin(CorsPlugin { cors ->
                    cors.addRule {
                ...
                app.unsafe.routes.get("/") { it.result("Hello") }
                val response = http.get("/", mapOf(ORIGIN to "https://example.com"))
    ...
    inner class ExposingHeaders {
        @Test
        fun `allows exposing a single header`() = JavalinTest.test(Javalin.create { cfg ->
            cfg.registerPlugin(CorsPlugin { cors ->
                cors.addRule {
                    it.reflectClientOrigin = true
        ...
        @Test
        fun `allows exposing multiple headers`() = JavalinTest.test(Javalin.create { cfg ->
            cfg.registerPlugin(CorsPlugin { cors ->
                cors.addRule {
                    it.reflectClientOrigin = true
                    it.exposeHeader("x-test")
    ...
    inner class ConvenienceFeatures {
        @Test
        fun `default scheme can be overridden`() = JavalinTest.test(Javalin.create { cfg ->
            cfg.registerPlugin(CorsPlugin { cors ->
                cors.addRule {
            ...
            app.unsafe.routes.get("/") { it.result("Hello") }
            val response = http.get("/", mapOf(ORIGIN to "http://example.com"))
            assertThat(response.header(ACCESS_CONTROL_ALLOW_ORIGIN)).isEqualTo("http://example.com")
            assertThat(response.body.string()).isEqualTo("Hello")
        }

        @Test
        fun `wildcard subdomain work`() = JavalinTest.test(Javalin.create { cfg ->
            cfg.registerPlugin(CorsPlugin { cors ->
                cors.addRule {
                    it.allowHost("*.example.com")
                }
            ...
            app.unsafe.routes.get("/") { it.result("Hello") }
            val response = http.get("/", mapOf(ORIGIN to "https://sub.example.com"))
            assertThat(response.header(ACCESS_CONTROL_ALLOW_ORIGIN)).isEqualTo("https://sub.example.com")
            assertThat(response.body.string()).isEqualTo("Hello")
        }

        @Test
        fun `default port detection works`() = JavalinTest.test(Javalin.create { cfg ->
            cfg.registerPlugin(CorsPlugin { cors ->
                cors.addRule {
        ...
        @Test
        fun `capitalization does not matter`() = JavalinTest.test(Javalin.create { cfg ->
            cfg.registerPlugin(CorsPlugin { cors ->
                cors.addRule {
        ...
        @Test
        fun `maxAge is present in preflight`() = JavalinTest.test(Javalin.create() { cfg ->
            cfg.registerPlugin(CorsPlugin { cors ->
                cors.addRule {
                    it.anyHost()
                    it.maxAge = 100
            ...
            app.unsafe.routes.get("/") { it.result("Hello") }
            val optionsResponse = http.options("/", mapOf(
                ACCESS_CONTROL_REQUEST_METHOD to "GET",
                ACCESS_CONTROL_REQUEST_HEADERS to "origin",
                ORIGIN to "https://example.com"
            ))
        ...
        @Test
        fun `maxAge is absent outside of preflight`() = JavalinTest.test(Javalin.create() { cfg ->
            cfg.registerPlugin(CorsPlugin { cors ->
                cors.addRule {
                    it.anyHost()
                    it.maxAge = 100
    ...
    inner class EdgeCases {
        ...
        @Test
        fun `cors plugin works with prefer405over404`() = JavalinTest.test(Javalin.create { cfg ->
            ...
            val optionsResponse = http.options("/", mapOf(ORIGIN to "https://example.com"))
            assertThat(optionsResponse.code())
                .isEqualTo(200)
            ...
            assertThat(getResponse.code())
                .isEqualTo(405)
            assertThat(getResponse.header(ACCESS_CONTROL_ALLOW_ORIGIN)).isEqualTo("https://example.com")
        }

        @Test
        fun `GH-2104 client sends dotless origin - wildcard configured`() = JavalinTest.test(Javalin.create { cfg ->
            cfg.registerPlugin(CorsPlugin { cors ->
                cors.addRule {
                    it.allowHost("*.example.com")
                }
    ...
    inner class ComplexSetup {
        @Test
        fun works() = JavalinTest.test(Javalin.create { cfg ->
            cfg.registerPlugin(CorsPlugin { cors ->
                cors
                    .addRule {
                        it.path = "images*"
        ...
        private fun checkHappyPath(http: HttpClient, input: String) {
            val response = http.get("/$input/media-id", mapOf(ORIGIN to "https://$input.local"))
            assertThat(response.header(ACCESS_CONTROL_ALLOW_ORIGIN)).isEqualTo("https://$input.local")
            assertThat(response.body.string()).isEqualTo("media-id")
        }

        private fun checkUnhappyPath(http: HttpClient, input: String) {
            val response = http.get("/$input/media-id", mapOf(ORIGIN to "https://example.local"))
            assertThat(response.header(ACCESS_CONTROL_ALLOW_ORIGIN)).isEmpty()
        }
    }
