...
import io.javalin.testing.htmlGet
...
class TestJavalinVue {
    ...
    @Test
    fun `vue component with state`() = JavalinTest.test(Javalin.create { config ->
    ...
    @Test
    fun `vue component without state`() = JavalinTest.test(Javalin.create { config ->
    ...
    @Test
    fun `vue component without state with pre renderer`() = JavalinTest.test(Javalin.create { config ->
        config.registerPlugin(JavalinVuePlugin { it.rootDirectory("src/test/resources/vue", Location.EXTERNAL) })
    ...
    @Test
    fun `vue component without state with post renderer`() = JavalinTest.test(Javalin.create { config ->
        config.registerPlugin(JavalinVuePlugin { it.rootDirectory("src/test/resources/vue", Location.EXTERNAL) })
        config.routes.get("/no-state", VueComponent("test-component", null, object : VueRenderer() {
    ...
    @Test
    fun `vue component without state with default renderer`() = JavalinTest.test(Javalin.create { config ->
        config.registerPlugin(JavalinVuePlugin { it.rootDirectory("src/test/resources/vue", Location.EXTERNAL) })
    ...
    @Test
    fun `vue component without state with pre and post renderer`() = JavalinTest.test(Javalin.create { config ->
        config.registerPlugin(JavalinVuePlugin { it.rootDirectory("src/test/resources/vue", Location.EXTERNAL) })
        config.routes.get("/no-state", VueComponent("test-component", null, object : VueRenderer() {
            override fun postRender(layout: String, ctx: Context): String {
                return layout + "POST_RENDER";
    ...
    @Test
    fun `vue3 component without state`() = VueJavalinTest.test({
    ...
    @Test
    fun `vue3 component with state`() = VueJavalinTest.test({
        it.vueInstanceNameInJs = "app"
        it.stateFunction = { state }
    }, null) { app, http ->
        val encodedState = """{"pathParams":{"my-param":"test-path-param"},"state":{"user":{"name":"tipsy","email":"tipsy@tipsy.tipsy"},"role":{"name":"Maintainer"}}}""".uriEncodeForJavascript()
    ...
    @Test
    fun `vue component with component-specific state`() = VueJavalinTest.test(null, null) { app, http ->
        val encodedEmptyState = """{"pathParams":{},"state":{}}""".uriEncodeForJavascript()
    ...
    @Test
    fun `vue component works Javalin#error`() = VueJavalinTest.test(null, null) { app, http ->
        app.unsafe.routes.get("/") { it.status(NOT_FOUND) }
        app.unsafe.routes.error(NOT_FOUND.code, "html", VueComponent("test-component"))
        assertThat(http.htmlGet("/").body.string()).contains("<body><test-component></test-component></body>")
    }

    @Test
    fun `unicode in template works`() = VueJavalinTest.test(null, null) { app, http ->
        app.unsafe.routes.get("/unicode", VueComponent("test-component"))
        assertThat(http.getBody("/unicode")).contains("<div>Test ÆØÅ</div>")
    }

    @Test
    fun `state is escaped`() = VueJavalinTest.test(null, null) { app, http ->
    ...
    @Test
    fun `component shorthand works`() = VueJavalinTest.test(null, null) { app, http ->
    ...
    @Test
    fun `non-existent component fails`() = VueJavalinTest.test({
    ...
    @Test
    fun `component can have attributes`() = VueJavalinTest.test(null, null) { app, http ->
    ...
    @Test
    fun `classpath rootDirectory works`() = VueJavalinTest.test({
    ...
    @Test
    fun `setting rootDirectory with Path works`() = VueJavalinTest.test({
    ...
    @Test
    fun `non-existent folder fails`() = VueJavalinTest.test({
        it.rootDirectory("/vue", Location.EXTERNAL)
    }, null) { app, http ->
        app.unsafe.routes.get("/fail", VueComponent("test-component"))
        assertThat(http.get("/fail").code()).isEqualTo(500)
    }


    @Test
    fun `@cdnWebjar resolves to webjar on localhost`() = VueJavalinTest.test({
        it.rootDirectory("src/test/resources/vue", Location.EXTERNAL)
    }, {
    ...
    @Test
    fun `@cdnWebjar resolves to cdn on non-localhost`() = VueJavalinTest.test({
        it.rootDirectory("src/test/resources/vue", Location.EXTERNAL)
    }, {
    ...
    @Test
    fun `@cdnWebjar resolves to https even on non https hosts`() = VueJavalinTest.test({
        it.rootDirectory("src/test/resources/vue", Location.EXTERNAL)
    }, {
    ...
    @Test
    fun `@inlineFile functionality works as expected if not-dev`() = VueJavalinTest.test({
        it.rootDirectory("src/test/resources/vue", Location.EXTERNAL)
    }, {
        it.contextResolver.url = { "http://123.123.123.123:1234/" }
    }) { app, http ->
    ...
    @Test
    fun `@inlineFile functionality works as expected if dev`() = VueJavalinTest.test({
        it.rootDirectory("src/test/resources/vue", Location.EXTERNAL)
    }, {
        it.contextResolver.url = { "http://localhost:1234/" }
    }) { app, http ->
    ...
    @Test
    fun `LoadableData class is included`() = VueJavalinTest.test(null, null) { app, http ->
        app.unsafe.routes.get("/shorthand", VueComponent("test-component"))
