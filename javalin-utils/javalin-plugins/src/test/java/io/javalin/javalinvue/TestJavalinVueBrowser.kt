...
class TestJavalinVueBrowser {
    ...
    @Test
    fun `loadabledata and state works when csp is enabled`() = VueJavalinTest.test({
    ...
    @Test
    fun `script tag without nonce is loaded if csp is not enabled`() = VueJavalinTest.test(null, null) { app, http ->
    ...
    @Test
    fun `script tag without nonce is not loaded if csp is enabled`() = VueJavalinTest.test({
        it.enableCspAndNonces = true
    }, null) { app, http ->
        app.unsafe.routes.get("/vue", VueComponent("test-component"))
        driver.get(http.origin + "/vue")
    ...
    @Test
    fun `path params are not html-encoded on JavalinVue`() = VueJavalinTest.test(null, null) { app, http ->
        app.unsafe.routes.get("/vue/{my-param}", VueComponent("test-component"))
        driver.get(http.origin + "/vue/odd&co")
        val pathParam = driver.executeScript("""return JavalinVue.pathParams["my-param"]""") as String
        assertThat(pathParam).isEqualTo("odd&co")
    ...
    @Test
    fun `the Vue prototype has the same values as JavalinVue`() = VueJavalinTest.test(null, null) { app, http ->
        app.unsafe.routes.get("/vue/{my-param}", VueComponent("test-component"))
        driver.get(http.origin + "/vue/odd&co")
    ...
    fun `script tags in state function does not break page rendering`() {
        ...
        VueJavalinTest.test({ it.stateFunction = { mapOf("some_key" to testValue) } }, null) { app, http ->
    ...
    @Test
    fun `LoadableData loads data correctly`() = JavalinTest.test(loadableDataTestApp()) { app, http ->
    ...
    @Test
    fun `LoadableData errors on bad url`() = JavalinTest.test(loadableDataTestApp()) { app, http ->
        driver.get(http.origin + "/ld")
    ...
    @Test
    fun `LoadableData cache can be enabled`() = JavalinTest.test(loadableDataTestApp()) { app, http ->
        driver.get(http.origin + "/ld")
        driver.executeScript("localStorage.clear()")
    ...
    @Test
    fun `LoadableData cache can be invalidated`() = JavalinTest.test(loadableDataTestApp()) { app, http ->
        driver.get(http.origin + "/ld")
        driver.executeScript("localStorage.clear()")
        driver.createLoadableData("ld", "new LoadableData('/api/users', {cache: true})")
        assertThat(driver.checkWindow("localStorage.length === 1")).isTrue()
    ...
    @Test
    fun `LoadableData errorCallback works`() = JavalinTest.test(loadableDataTestApp()) { app, http ->
        driver.get(http.origin + "/ld")
    ...
    @Test
    fun `LoadableData instance can refresh itself`() = JavalinTest.test(loadableDataTestApp()) { app, http ->
        driver.get(http.origin + "/ld")
        driver.createLoadableData("ld", "new LoadableData('/api/users')")
        driver.createMaryOnBackend()
        driver.executeScript("window.ld.refresh()")
    ...
    @Test
    fun `LoadableData instance can refresh instances with same URL`() = JavalinTest.test(loadableDataTestApp()) { app, http ->
        driver.get(http.origin + "/ld")
    ...
    @Test
    fun `LoadableData instance can not refresh instances with different URL`() = JavalinTest.test(loadableDataTestApp()) { app, http ->
        driver.get(http.origin + "/ld")
        driver.createLoadableData("ld1", "new LoadableData('/api/users')")
